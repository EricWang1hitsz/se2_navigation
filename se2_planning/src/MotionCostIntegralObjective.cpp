    ...
    : OptimizationObjective(si), interpolateMotionCost_(enableMotionCostInterpolation)
{
    ...
    traversability_map_sub_ = nh_.subscribe("/traversability_estimation/traversability_map", 100, &motionCostIntegralObjective::traversabilityMapCallback, this);

    ROS_WARN("Motion cost objective initialized. ");
}
...
void ompl::base::motionCostIntegralObjective::traversabilityMapCallback(const grid_map_msgs::GridMapPtr &traversability_map)
{
    ...
    ROS_INFO("Motion optimazation receive traversability map successfully.");
}
